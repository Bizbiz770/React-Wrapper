...
describe('getFormattedCityWeatherList function use-cases', () => {
    test('return formatted weather list if response data list is not empty', () => {
        const expectedResult = [{
                ...
                description: "overcast clouds",
                temp: 271.49,
                time: moment_1.default("2021-01-30 21:00:00").format('LT'),
                date: Utils_1.formatWeatherDate("2021-01-30 21:00:00")
            }];
        expect(Home_1.default.getFormattedCityWeatherList(responseToTest)).toEqual(expectedResult);
    });
    test('return formatted weather list empty if response data list is empty', () => {
        expect(Home_1.default.getFormattedCityWeatherList(emptyResponseToTest)).toEqual([]);
    });
});
